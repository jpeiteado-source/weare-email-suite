...
export default async function handler(req, res) {
  try {
    if (req.method === 'GET') {
      ...
      if (month) {
        const raw = await redis(['GET', `plan:${nombre}:${month}`]);
        if (!raw) return res.status(404).json({ error: 'Plan no encontrado' });
        return res.status(200).json(JSON.parse(raw));
      }

      const all = (await redis(['SMEMBERS', `plans:index:${nombre}`])) || [];
      ...
      const results = await pipeline(top.map(m => ['GET', `plan:${nombre}:${m}`]));
      const plans = results
    ...
    if (req.method === 'POST') {
      ...
      await pipeline([
        ['SET', `plan:${nombre}:${month}`, JSON.stringify(plan)],
        ['SADD', `plans:index:${nombre}`, month]
      ]);
    ...
    if (req.method === 'DELETE') {
      ...
      if (!nombre || !month) return res.status(400).json({ error: 'Falta nombre o mes' });
      await pipeline([
        ['DEL', `plan:${nombre}:${month}`],
        ['SREM', `plans:index:${nombre}`, month]
      ]);
